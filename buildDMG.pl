#!/usr/bin/perl -w
#
my $versionStr = '$Id: buildDMG.pl,v 1.1 2006/01/15 09:00:37 aufflick Exp $';
#
...
# Customisations added for SkypeMenuX
use Mac::OSA::Simple;

my $appleScript = <<EOS;
tell application "Finder"
open disk "$volName"
set the dmg_window to (front window)
set the current view of dmg_window to icon view
set the toolbar visible of dmg_window to false
set the win_options to (icon view options of dmg_window)
set the icon size of win_options to 100
set the arrangement of win_options to arranged by kind
set the bounds of dmg_window to {33, 68, 397, 271}
set the position of dmg_window to {33, 68}
end tell
tell application "Xcode"
display dialog "Please setup the window in the Finder how you like it. I know it's dumb, but the Finder doesn't seem to save all the window view changes made via AppleScript unless it notices human input! Click OK when done"
-- need to click on the window here - find out why this is necessary or how to simulate it!!
end tell
EOS

print "$appleScript" if $debug;
...
if ($compressionLevel) {
    ...
    $output = `hdiutil convert -tasks 1 -verbose "$buildDir/$tmpDmgName" -format UDZO -imagekey zlib-level=$compressionLevel -o "$buildDir/$dmgName"`;
